...
import sns as sns # standard import, but using your verified seaborn configuration
...
def load_data():
    ...
    # Force uniformity across the raw category strings to prevent duplication bugs
    df['main_category_clean'] = df['main_category'].astype(str).str.strip()
    
    # ============================================================
    # DATA ALIGNMENT ENGINE: RE-MAPPING TO Broad PRESENTATION GROUPS
    # ============================================================
    def assign_macro_group(cat):
        c = cat.lower()
        if 'beverage' in c or 'drink' in c or 'juices' in c or 'soda' in c:
            return 'Beverages'
        elif 'asian style' in c or 'ready meal' in c or 'noodle' in c:
            return 'Asian Style Ready Meals'
        elif 'snack' in c or 'confectionery' in c or 'biscuit' in c or 'cookie' in c or 'chocolate' in c or 'sweet' in c:
            return 'Snacks & Confectionery'
        elif 'dairy' in c or 'milk' in c or 'cheese' in c or 'egg' in c or 'yogurt' in c or 'cream' in c:
            return 'Dairy & Eggs'
        elif 'meat' in c or 'fish' in c or 'seafood' in c or 'poultry' in c or 'chicken' in c:
            return 'Meats & Seafood'
        elif 'plant-based' in c or 'lentil' in c or 'bean' in c or 'legume' in c or 'veggie' in c:
            return 'Plant-Based Foods'
        else:
            return 'Other Product Categories'

    df['macro_category'] = df['main_category_clean'].apply(assign_macro_group)
    return df
...
# Pull the exact unique categories generated by our mapping engine
macro_options = sorted(df['macro_category'].unique().tolist())

# Keep 'All' at the top of the selection array
if 'Other Product Categories' in macro_options:
    macro_options.remove('Other Product Categories')
    macro_options.append('Other Product Categories')

categories = ['All'] + macro_options
selected_category = st.sidebar.selectbox("Select Product Category", categories)

# Filter dataset dynamically based on user selection
if selected_category != 'All':
    filtered_df = df[df['macro_category'] == selected_category]
else:
    filtered_df = df
