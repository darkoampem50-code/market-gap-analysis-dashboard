...
@st.cache_data
def load_data():
    # If Streamlit finds the file, we read it
...
try:
    ...
    # ============================================================
    # 3. DYNAMIC SIDEBAR FILTERING (NO MORE GUESSWORK)
    # ============================================================
    st.sidebar.header("Filter Options")
    
    # Extract the actual raw categories present in your file, dropping blanks
    raw_categories = df['main_category'].dropna().unique().tolist()
    
    # Clean up whitespace and sort them
    cleaned_categories = sorted(list(set([str(cat).strip() for cat in raw_categories if str(cat).strip() != ""])))
    
    # Build the filter list with 'All' at the top
    categories = ['All'] + cleaned_categories
    selected_category = st.sidebar.selectbox("Select Product Category", categories)
    
    # Filter dataset dynamically based on user selection
    if selected_category != 'All':
        filtered_df = df[df['main_category'].str.strip() == selected_category]
    else:
        filtered_df = df
    ...
    if "asian" in str(selected_category).lower():
        st.success("""
    ...
    st.error("🚨 Critical Error: 'cleaned_market_gap_data.csv' was not found in the root app repository directory. Please make sure the data file is committed to GitHub alongside this script file.")
